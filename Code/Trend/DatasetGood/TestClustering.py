import libs.Models.Trend.ClusteringModel as ClusteringModel
...
timeseries.extend([{'y': DataGen.getTimeSeries(paramsNoTrend[e]), 'trend': False, 'trend_startingpoint': None} for e in paramsNoTrend])
...
mk = ClusteringModel.ClusteringModel(timeseries, [], [])
roc_table, trendstart_scores = mk.fit()
roc_table = [{"parameter":e, "value":roc_table[e]} for e in roc_table]
trendstart_scores = [{"parameter":e, "value":trendstart_scores[e]} for e in trendstart_scores]

result.append({"model": "Clustering", "dataset": "ParamGood", "roc_table": roc_table,
