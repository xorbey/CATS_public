...
class MannKendallModel(ModelParent.ModelParent):
    def __init__(self, trainDfs: List[pd.DataFrame], testX: np.array, testy: np.array):
        ...
        #self.parameterChoices = [element/100 for element in list(range(0,100,2))]
        ...
        for counter, parameterChoice in enumerate(self.parameterChoices):
            ...
            for counter,df in enumerate(self.trainDfs):
                ...
                for point_counter, datapoint in enumerate(testdf):
                    ...
                    result = mk.hamed_rao_modification_test(trainData, alpha=parameterChoice)
                    if predicted_trend == False:
                        predicted_trend = result.h
...
#                 dataFig.show()
    ...
    def predict(self, values: np.ndarray, parameterChoice) -> np.ndarray:
        result = mk.hamed_rao_modification_test(values, alpha=parameterChoice)
        return result.h
