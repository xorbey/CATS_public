...
class IForest(ModelParent):
    """
    Isolation Forest class, with flexible setting of the anomaly contamination in the training data and the number of
    of base estimators
    """
    def __init__(self, trainX: np.array, testX: np.array, testy: np.array, n_estimators=100, contamination=0.01, split = 5):
        """
        ...
        self.recall = 0
        self.predictions = []
        self.modelName = "IsolationForest"
        self.split = split
    ...
    def fit(self) -> None:
        ...
        self.model = IsolationForest(n_estimators=self.n_estimators, contamination=self.contamination).fit(trainXX)
    ...
    def showResults(self):
        ...
        fig.add_trace(go.Scatter(x=x, y=y, marker_color="orange", name="IsolationForest"))
        title = "IF Recall: " + str(self.recall) + " Precision: " + str(
            self.precision) + " n_estimators: " + str(self.n_estimators) + " contamination: " + \
            str(self.contamination) + "\n"
        fig.update_layout(title=title)
        fig.show()

    def saveModel(self, path:Path):
        jsonpath = path / self.modelName + ".json"
        modelpath = path / self.modelName + ".sav"
        jsondic = {"n_estimators":self.n_estimators, "contamination":self.contamination, "modelpath":str(modelpath.absolute())}
        with open(str(jsonpath.absolute()), 'w') as jsonpath:
            json.dump(jsondic, jsonpath)
        pickle.dump(self.model, open(modelpath, 'wb'))

    def loadModel(self, path:Path):
        jsonpath = path / self.modelName + ".json"
        with open(str(jsonpath.absolute())) as f:
            data = json.load(f)
