...
class OCSVM(ModelParent):
    def __init__(self, trainX: np.array, testX: np.array, testy: np.array, kernel='rbf', nu=0.7, split = 5):
        """
        :param trainX: Training data
        :param testX: Test data
        :param testy: Labels of the test data
        :param kernel: Kernel to be used in the algorithm
        :param nu: upper bound on the fraction of training errors and a lower bound of the fraction of support vectors.
    ...
    def fit(self) -> None:
        ...
        self.model = OneClassSVM(kernel=self.kernel, nu=self.nu, gamma="auto", degree=3).fit(trainXX)
    ...
    def showResults(self):
        """
        Plots the OCSCVM Decision Function
        ...
        fig.add_trace(go.Scatter(x=x, y=y, marker_color="orange", name="OCSVM"))
        title = "OCSVM Recall: " + str(self.recall) + " Precision: " + str(
            self.precision) + " Kernel: " + str(self.kernel) + " Nu: " + str(self.nu) + "\n"
