...
fig.add_trace(go.Scatter(x = [e for e in range(len(featuresBMW))], y = featuresBMW, name = "Values", marker_color = "blue", showlegend = False), row = 1, col = 3)
fig.add_trace(go.Scatter(x = colorsXAnomaly, y = colorsYAnomaly,
                           marker_color = "orange", mode = "markers", name = "Predicted anomalies", showlegend = False), row = 1, col = 3)
fig.add_trace(go.Scatter(x = decisionX, y = [20 for e in range(len(decisionX))],
...
fig.show()
path = "G:/Modelle_Paper/Plots/Figures/anomalycomparisonBMWSTD.pdf"
